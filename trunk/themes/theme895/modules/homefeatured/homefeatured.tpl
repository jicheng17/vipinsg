...
<div id="featured-products_block_center" class="main_page_products block">
	...
	{if isset($products) AND $products}
			...
			{foreach from=$products item=product name=homeFeaturedProducts}
				...
				<li class="ajax_block_product col-xs-12 col-sm-6 col-md-4 {if $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 1} first_item_of_line {/if} {if $smarty.foreach.homeFeaturedProducts.iteration > ($smarty.foreach.homeFeaturedProducts.total - $totModulo)}last_line{/if} {if $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLineTablet == 0}last_item_of_tablet_line{elseif $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLineTablet == 1}first_item_of_tablet_line{/if} {if $smarty.foreach.homeFeaturedProducts.iteration > ($smarty.foreach.homeFeaturedProducts.total - $totModuloTablet)}last_tablet_line{/if} list-product-{$smarty.foreach.homeFeaturedProducts.iteration}">
                <div class="inner_content clearfix">
                    ...
                    <div class="label-box">
                    {if isset($product.new) && $product.new == 1}<div class="new-box"><span class="new">{l s='New' mod='homefeatured'}</span></div>{/if}
                	{if isset($product.reduction) && $product.reduction && isset($product.on_sale) && $product.on_sale && isset($product.show_price) && $product.show_price && !$PS_CATALOG_MODE}<div class="sale-box"><span class="on_sale">{l s='Sale'}</span></div>{/if}
                    </div>
                    <div class="product_content">
                    {if $product.show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
                        <p class="price_container">
                            <span class="price{if isset($product.specific_prices) && $product.specific_prices} price-new{/if}">{if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if}</span>
                    ...
                    {/if}
					<h5><a class="product_link" href="{$product.link|escape:'html'}" title="{$product.name|truncate:50:'...'|escape:'htmlall':'UTF-8'}">{$product.name|truncate:30:'...'|escape:'htmlall':'UTF-8'}</a></h5>
