...
<section id="bestseller_home" class="block main_page_products">
  ...
  <ul class="row">
    ...
    <p class="product_desc">{$product.description_short|strip_tags|truncate:65:'...'}</p>
	  ...
	  {/if}
      <a class="btn btn-default exclusive ajax_add_to_cart_button btn_add_cart" rel="ajax_id_product_{$product.id_product}" href="{$link->getPageLink('cart')}?qty=1&amp;id_product={$product.id_product}&amp;token={$static_token}&amp;add" title="{l s='Add to cart' mod='homefeatured'}"><span>{l s='Add to cart' mod='homefeatured'}</span></a> 
      <a class="btn btn-default button" href="{$product.link|escape:'html'}" title="{l s='View' mod='blockbestsellers'}">{l s='View' mod='blockbestsellers'}</a>
      </div>
      </div>
      </li>
      {if $smarty.foreach.myLoop.iteration>2}{break}{/if}
    {/foreach}
  </ul>
  {else}
